...
public class PantallaPrincipalController extends Controller implements Initializable {
    ...
    private void aplicarFiltro(String filtro) {
        ...
        // Ordenar huellas por fecha
        huellas.sort(Comparator.comparing(Huella::getFecha));

        LocalDate hoy = LocalDate.now();
        LocalDate haceUnaSemana = hoy.minusDays(6); // Incluye los últimos 7 días

        DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");

        for (int i = 0; i < huellas.size(); i++) {
            Huella huella = huellas.get(i);
            LocalDate fechaHuella = LocalDate.parse(huella.getFecha().toString(), formatter);
            BigDecimal factorEmisionL = (i < factoresEmision.size()) ? factoresEmision.get(i) : BigDecimal.ZERO;

            if ("Todos".equals(filtro)) {
                series.getData().add(new XYChart.Data<>(fechaHuella.toString(), factorEmisionL.doubleValue()));

            } else if ("Mes".equals(filtro)) {
                if (fechaHuella.getMonth() == hoy.getMonth() && fechaHuella.getYear() == hoy.getYear()) {
                    series.getData().add(new XYChart.Data<>(fechaHuella.toString(), factorEmisionL.doubleValue()));
                }

            } else if ("Semana".equals(filtro)) {
                if (!fechaHuella.isBefore(haceUnaSemana) && !fechaHuella.isAfter(hoy)) {
                    series.getData().add(new XYChart.Data<>(fechaHuella.toString(), factorEmisionL.doubleValue()));
                }
            }
        }
    ...
    public void generarPDF() throws IOException, DocumentException {
        ...
        List<Usuario> users = userService.traerUsuarios();
